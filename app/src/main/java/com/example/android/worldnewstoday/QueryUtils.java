...
public final class QueryUtils {
    ...
    /**
     * Query the website dataset and return a list of {@link News} objects.
     */
    public static List<News> fetchNewsData(String requestUrl) {
        // Create URL object

        Log.i(LOG_TAG, "Creating URL Object to fetch news data");
    ...
    private static List<News> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            for (int i = 0; i < results.length(); i++){
                JSONObject thenews = results.getJSONObject(i);
                String title = thenews.getString("webTitle");
                String url = thenews.getString("webUrl");
                String date = thenews.getString("webPublicationDate");
                String category = thenews.getString("sectionName");
                // Create a new {@link Football} object with the magnitude, location, time,
                // and url from the JSON response.
                News newsStories = new News(title,url,date,category);
                // Add the new {@link Football} to the list of footballs.
                news.add(newsStories);
            }
